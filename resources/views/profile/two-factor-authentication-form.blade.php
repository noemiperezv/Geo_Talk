<x-jet-action-section>
    ...
    <x-slot name="description">
        {{ __('Agrega seguridad adicional a tu cuenta utilizando autentificación por dos factores.') }}
    </x-slot>

    <x-slot name="content">
        <h3 class="h5 font-weight-bold">
            @if ($this->enabled)
                {{ __('Has habilitado la auntentificación por dos factores.') }}
            @else
                {{ __('No has habilitado la autentificación por dos factores.') }}
            @endif
        </h3>
